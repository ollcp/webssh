...
export default {
    ...
    computed: {
        ...mapState(['currentTab']),
        uploadUrl: () => {
            return `${process.env.NODE_ENV === 'production' ? `${location.origin}` : 'api'}/wwssadad/file/upload`
        },
        uploadData: function() {
            return {
    ...
    methods: {
        ...
        uploadProgress(e, f) {
            ...
            if (e.percent === 50) {
                const ws = new WebSocket(`${(location.protocol === 'http:' ? 'ws' : 'wss')}://${location.host}${process.env.NODE_ENV === 'production' ? '' : '/ws'}/wwssadad/file/progress?id=${f.uid}`)
                ws.onmessage = e1 => {
        ...
        downloadFile() {
            ...
            const downloadUrl = `${prefix}/wwssadad/file/download?path=${this.downloadFilePath}&sshInfo=${this.$store.getters.sshReq}`
